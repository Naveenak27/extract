...
const extractEmails = (html, pageUrl) => {
  ...
  // Expanded HR/career-related keywords for classification
  const hrKeywords = [
    // Basic HR terms
  ...
  allEmails.forEach(email => {
    // Check if email appears in an HR-related context
    ...
    // Check email prefix for HR keywords
    const localPart = email.split('@')[0].toLowerCase();
    if (hrKeywords.some(keyword => localPart.includes(keyword))) {
      isHrRelated = true;
    }
    
    // Common HR email pattern checks (e.g., jobs@, careers@, etc.)
...
const prioritizeUrls = (urls) => {
  ...
  const highPriorityPatterns = [
    /career/i, /job/i, /vacancy/i, /recruit/i, /employment/i, /hiring/i, /join.*?us/i
  ];
  
  const mediumPriorityPatterns = [
    /contact/i, /about.*?us/i, /team/i, /people/i, /staff/i, /directory/i, /department/i
  ];
